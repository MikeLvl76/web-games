...
export default function Menu({ onStart }: Props) {
  ...
  const [oppColor, setOppColor] = useState<NonNullable<Token>>("yellow");
    ...
    <div className="flex flex-col items-center gap-4 w-[80vw] h-[80vh] rounded-md p-2 select-none bg-slate-300">
      <div className="flex flex-row w-full h-[50%] justify-around items-center">
        <div className="flex flex-col gap-2 p-2 items-center justify-center">
          ...
          <Select
            value={playerColor}
            onValueChange={(value) => {
              setPlayerColor(value as NonNullable<Token>);
              setOppColor(
                (value as NonNullable<Token>) === "red" ? "yellow" : "red"
              );
            }}
          >
      ...
      <div className="flex justify-center items-end w-full h-[20%]">
        <Button
          ...
          onClick={() => onStart({ playerColor, oppColor, enableTime })}
        >
          Start game
        </Button>
      </div>
