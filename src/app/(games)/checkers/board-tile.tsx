...
const BoardTile = memo((props: Props) => {
  ...
  const canMoveHere =
    props.currentPlayer.nextMove?.indices.includes(props.currentIndex) &&
    props.selectedIndex !== -1 &&
    props.tiles[props.selectedIndex].piece?.color === props.currentPlayer.color;
  ...
  const tileContentClasses = [
    ...
    canMoveHere
      ...
      : props.currentTile.piece?.color === "black"
      ? "bg-black w-12 h-12"
    ...
    >
      {(canMoveHere || props.currentTile.piece) && (
        <div className={tileContentClasses}>
          {props.currentTile.piece?.type === "dame" && (
